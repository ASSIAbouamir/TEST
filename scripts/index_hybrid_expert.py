...
def build_bm25_index(nodes: List[Dict]) -> BM25Okapi:
    print(f"[BM25] Tokenising {len(nodes)} nodes …")
    corpus = []
    for n in nodes:
...
def build_dense_index(nodes: List[Dict]) -> faiss.IndexFlatIP:
    print(f"[FAISS] Encoding {len(nodes)} nodes with {EMBED_MODEL} …")
...
def build_graph_index(edges: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    """
    print(f"[GRAPH] Building adjacency list from {len(edges)} edges …")
...
def save_node_lookup(nodes: List[Dict]):
    """Save ordered node list and id→index map for fast lookup."""
...
if __name__ == "__main__":
    import sys
    # Si un argument est passé, on l'utilise comme DATA_DIR
    if len(sys.argv) > 1:
        DATA_DIR = sys.argv[1]
